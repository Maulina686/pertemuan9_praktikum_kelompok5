...
import 'package:pertemuan9_praktikum_kelompok5/product_favorite.dart'
    hide demoProducts, Product;  // tetap diperlukan untuk menghindari konflik nama
...
import 'package:pertemuan9_praktikum_kelompok5/providers/product_provider.dart';  // ← import baru
    ...
    return Container(
      ...
      child: const Text.rich(
        TextSpan(
          ...
          children: [
            TextSpan(text: "Ketapang bahagia\n"),
            TextSpan(
              text: "Cashback 20%",
      ...
      builder: (context, productProvider, child) {
        ...
        return Column(
          children: [
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 20),
              child: SectionTitle(title: "produk paling di cari", press: () {}),
            ),
            Padding(
...
// ==============================
// Recently Added Products – juga dari API (misal ambil 4 produk pertama)
// ==============================
class RecentlyAddedProducts extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<ProductProvider>(
      builder: (context, productProvider, child) {
        if (productProvider.isLoading) {
          return const SizedBox.shrink(); // tidak tampil saat loading
        }
        final allProducts = productProvider.products;
        // Ambil beberapa produk terbaru (asumsikan API mengembalikan terbaru di awal)
        final recentProducts = allProducts.length > 4
        ...
        return Column(
          children: [
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 20),
              child: SectionTitle(title: "terakhir di tambahkan", press: () {}),
            ),
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: GridView.builder(
...
const cartIcon = '''<svg width="22" height="18" viewBox="0 0 22 18" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const bellIcon = '''<svg width="15" height="20" viewBox="0 0 15 20" fill="none" xmlns="http://www.w3.org/2000/svg">
